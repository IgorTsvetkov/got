<template>
  <div class="bg-secondary text-dark-50 shadow rounded p-3 m-3 w-75">
    <div v-if="min<max" class="form-group ">
      <label
      ...
      <div v-if="canBet">
        <div class="shadow p-2 shadow bg-dark text-light rounded">
          ...
          <div>
          <div class="btn btn-dark mt-2 width-100 d-flex justify-content-center" @click="toggleExpend">
            <img src="/web/images/arrow_down.svg" :class="{'rotate-180':expend}" width="20vw" height="15vw" />
          </div>
          <div v-if="expend">
          <div class="d-flex">
            {{min}}
...
export default {
  ...
  methods: {
    makeBet(valueBet=this.current){
      this.$axios.post(`/auction/bet/player_id=${this.player_id}&cost=${valueBet}`)
    },
    leave() {
